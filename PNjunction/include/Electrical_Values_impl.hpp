...
// ---------------------------------------- DOPING VALUES ------------------------------------------------------------------------
// this method return the value of the doping of the pn junction: if we are in the first half (x-axis) of
// the domain the doping in positive otherwise is negative
...
double PotentialValues<dim>::value(const Point<dim> & p, const unsigned int component) const{
  ...
  if (p[0] <= 0.45*L) // check the x component of the point p
    return V_TH*std::log(N1/ni);
  else
    return V_TH*std::log(N2/ni);

}

// --------------------------------------- ELECTRON INITIAL VALUES -------------------------------------------------------------
// This method return the value of the electron density, I think we initilaize the first 45% of the domain to
// N1 density and the other 55% to N2
