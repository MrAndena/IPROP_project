...
InsIMEX<dim>::InsIMEX(parallel::distributed::Triangulation<dim> &tria, const NavierStokes& NS, unsigned short int i)
    ...
    degree(NS.numerical_parameters.FE_degree),
    NS_struct(NS),
    simulation_tag(i),
    triangulation(tria),
...
void InsIMEX<dim>::output_results(const unsigned int output_index) const
{
    ...
    std::string output_directory = base_directory + "/NS_Simulation_"+std::to_string(simulation_tag);
    ...
    {
        // List all the .vtu files for the current timestep
        std::vector<std::string> vtu_filenames;
        for (unsigned int i = 0; i < Utilities::MPI::n_mpi_processes(mpi_communicator); ++i)
        {
            vtu_filenames.push_back(basename + Utilities::int_to_string(i, 4) + ".vtu");
        }

        // Create the .pvtu file that references all the .vtu files
        std::string pvtu_filename = output_directory + "/" + basename + ".pvtu";
        std::ofstream pvtu_output(pvtu_filename);
        data_out.write_pvtu_record(pvtu_output, vtu_filenames);

        // Add the .pvtu file to the .pvd record
        times_and_names.push_back({time.current(), basename + ".pvtu"});

        // Write the PVD file
        std::ofstream pvd_output(output_directory + "/navierstokes.pvd");
